...
def write_prompt_md(repo_root: Path, files: list[Path], out_dir: Path, bundle_files: list[Path]) -> Path:
...
You are reviewing a project bundle provided as {bundle_ref}. Treat it as read-only source; paths reflect the original repo.{bundle_note}
...
def main():
    ...
    prompt_path = write_prompt_md(repo_root, files, out_dir, bundle_paths)
